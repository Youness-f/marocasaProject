...
if (isset($_POST['submit'])) {
  if (
    ...
    empty($_POST['beds']) || empty($_POST['bath']) || empty($_POST['sq_ft']) || empty($_POST['home_type']) ||
    empty($_POST['year_built']) || empty($_POST['type']) || empty($_POST['description']) || empty($_POST['price_sqft'])
  ) {
    ...
    try {
      $insert = $conn->prepare("INSERT INTO props(name, location, price, beds, bath, sq_ft, home_type, year_built, type, description, price_sqft, admin_name, image)
     VALUES (:name, :location, :price, :beds, :bath, :sq_ft, :home_type, :year_built, :type, :description, :price_sqft, :adminname, :image)");
      $insert->execute(
        [
          ':name' => $name,
          ':location' => $location,
    ...
    <div class="card">
      <div class="card-body">
        ...
        <form method="POST" action="create-properties.php" enctype="multipart/form-data">
          ...
          <div class="form-outline mb-4 mt-4">
            <input type="text" name="bath" id="form2Example1" class="form-control" placeholder="bath" />
          </div>
          <div class="form-outline mb-4 mt-4">
