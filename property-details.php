...
//display images for gallery
$image = $conn->query("SELECT * FROM related_images WHERE prop_id='$id'");
$image->execute();
$allImages = $image->fetchAll(PDO::FETCH_OBJ);

//check if user add curent property to favs
...
?>
<div class="site-blocks-cover inner-page-cover overlay" style="background-image: url(<?php echo THUMBNAILSURL; ?>/<?php echo $allDetails->image; ?>);" data-aos="fade" data-stellar-background-ratio="0.5">
  <div class="container">
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-lg-4">
        ...
        <div class="bg-white widget border rounded">
          ...
          <?php if (isset($_SESSION['user_id'])) : ?>
            <?php if ($check_request->rowCount() > 0) :  ?>
              <p class="text-center">You have already requested for this property</p>
            <?php else : ?>
              <form action="process/process-request.php" method="POST" class="form-contact-agent">
            ...
            <?php endif; ?>
          <?php else : ?>
            <p class="text-center">Please <a href="auth/login.php">login</a> to contact agent</p>
          <?php endif; ?>
        </div>
        ...
        <div class="bg-white widget border rounded">
          <h3 class="h4 text-black widget-title mb-3 ml-0">Add To Favorite</h3>
          <div class="px-3" style="margin-left: -15px;">
            <?php if (isset($_SESSION['user_id'])) : ?>
              <form action="favs/add-fav.php" method="POST" class="form-contact-agent">
                ...
                <?php if ($check->rowCount() > 0) : ?>
                  <div class="form-group">
                    <a href="favs/delete-fav.php?prop_id=<?php echo $id; ?>&user_id=<?php echo $_SESSION["user_id"]; ?>" class="btn btn-primary text-white">remove from favorite</a>
                  </div>
                <?php else : ?>
                  <div class="form-group">
                    <input type="submit" id="phone" name="submit" class="btn btn-primary" value="Add to favorite">
                  </div>
                <?php endif; ?>
              </form>
            <?php else : ?>
              <p class="text-center">Please <a href="auth/login.php">login</a> to add to favorite</p>
            <?php endif; ?>
          </div>
        </div>
          ...
          <div class="property-entry h-100">
            <a href="property-details.php?id=<?php echo $allRelatedProp->id; ?>" class="property-thumbnail">
              <div class="offer-type-wrap">
                <span class="offer-type bg-<?php if ($allRelatedProp->type == "rent") {
                                              echo "success";
                                            } else {
                                              echo "danger";
                                            } ?>"><?php echo $allRelatedProp->type; ?></span>
              </div>
              <img src="<?php echo THUMBNAILSURL ; ?>/<?php echo $allRelatedProp->image; ?>" alt="Image" class="img-fluid">
            </a>
